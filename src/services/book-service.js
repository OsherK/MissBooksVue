...
const API_KEY = 'AIzaSyACwwEsXhpJ_uoYOS0P38LlFhcPCQyTKws';
...
function _loadBooks() {
    books = storageService.loadFromStorage('books');
    if (!books) books = initBooks();
}
...
async function searchGoogleBooks(searchVal){
    try{
        if(!searchVal) return null;
        const res = await axios.get(url + searchVal)
...
function addGoogleBook(book){
    const bookToSave = {
        ...
        description: book.searchInfo.textSnippet || book.volumeInfo.description,
        pageCount: book.volumeInfo.pageCount,
        thumbnail: book.volumeInfo.imageLinks.thumbnail,
        language: book.volumeInfo.language,
        listPrice: {
            amount: utilService.getRandomInteger(150),
            currencyCode: 'USD',
    ...
    _saveBooks()
    return JSON.parse(JSON.stringify(bookToSave));
}
...
function initBooks() {
    return [{
            ...
            "thumbnail": "https://www.seenox.org/wp-content/uploads/2014/03/Funny-Worst-Book-Titles-And-Covers-01.jpg",
            "language": "en",
            "listPrice": {
                "amount": 109,
                "currencyCode": "EUR",
        ...
        },
        {
            ...
            "thumbnail": "https://justsomething.co/wp-content/uploads/2017/03/15-children-s-book-covers-photoshopped-in-hilarious-ways-12.jpg",
            "language": "sp",
            "listPrice": {
                "amount": 44,
                "currencyCode": "EUR",
...
async function addReview(review, bookId){
    const book = await getById(bookId);
    review.id = utilService.makeId();
    book.reviews = book.reviews ? [...book.reviews, review] : [review];
    books = books.map(currBook => currBook.id === bookId ? book : currBook);
    _saveBooks();
